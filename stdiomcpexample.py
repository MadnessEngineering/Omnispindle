...
from mcp.client.sse import sse_client  # If using SSE transport
import os
...
async def run_with_sse():
    url = "http://localhost:8000/sse"  # Adjust to your server's SSE endpoint
    headers = {"Content-Type": "application/json"}

    async with sse_client(url, headers=headers) as (read, write):
        async with ClientSession(read, write) as session:
            # Rest of the agent_loop implementation remains the same
            prompt = "What are my todo items?"  # Adjust prompt for your todo server
            res = await agent_loop(prompt, client, session)
            return res


if __name__ == "__main__":
    res = asyncio.run(run_with_sse())
    print(res.text)
